...
module.exports = {
  ...
  /**
   * Used in order to get additional options for importing of RAML 0.8 schema
   *
  ...
  getOptions: function (mode = 'document') {
    let optsArray = [
      ...
      {
        name: 'Use RAML 1.0 conversion flow',
        id: 'convertWith10',
        type: 'boolean',
        default: false,
        description: 'Choose whether to use better supported RAML 1.0 conversion flow via converting RAML 0.8 schema' +
        ' to RAML 1.0 first and then to collection.',
        external: true
      }
    ];
