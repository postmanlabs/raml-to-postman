...
var raml1ToPostman = require('raml1-to-postman');
var webApiParser = require('webapi-parser').WebApiParser;
...
var converter = {
    ...
    convertWith10: function (data, options, callback) {
        try {
            webApiParser.raml08.parse(data)
                .then((parsedData) => {
                    return webApiParser.raml10.generateString(parsedData);
                })
                .then((convertedApi) => {
                    raml1ToPostman.convert({
                        type: 'string',
                        data: convertedApi
                      }, options, (error, result) => {
                        if (error) {
                          return callback('Error importing RAML 0.8 spec');
                        }
